...
def dropVertice(conjunto_vertices, conjunto_arestas, grafo):
    ...
    vertice = int(input(f'Dado V(G): {conjunto_vertices}, deseja remover qual?\n'))
    ...
    for coluna in grafo:
        auxColuna = list(coluna)
        del auxColuna[indexRemocao]
        auxColuna = np.array(auxColuna)
        auxGrafo.append(auxColuna)  

    del auxGrafo[indexRemocao]
    conjunto_vertices.remove(vertice)
    return conjunto_vertices, auxArestas, np.array(auxGrafo)
...
def consultarVertice(grafo, conjunto_vertices):
    ...
    vertice = int(input(f"Dado V(G): {conjunto_vertices}, deseja visitar qual?\n"))
